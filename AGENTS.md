...
Temporary Titan Token (`t3admin`) lets WordPress admins (or network super-admins on Multisite) temporarily replace a user's effective role set for a fixed window of time — either a specific date/time, or a duration from now (minutes, hours, days). This can elevate, demote, or remove roles entirely for the duration. When the window closes the user's original role set is automatically restored. Every grant, revocation, and expiry is appended to a JSONL audit log.
...
│   └── class-cli.php                   T3Admin_CLI — WP-CLI grant and revoke commands.
...
array(
    'id'             => string,  // UUID v4
    'user_id'        => int,
    'original_roles' => string[], // role slugs before the temporary change
    'temporary_roles'=> string[], // role slugs during the temporary change
    // legacy convenience fields retained for single-role displays/logs:
...
2. **`user_has_cap` filter** — On every capability check WordPress passes the user object through this filter. If the user holds an active-but-overdue grant the role-set change is resolved inline. This is the safety net: even if the cron event was missed (low-traffic site, cron backlog) the temporary role set cannot persist beyond the first request that checks capabilities.
...
The Users list table Role column is replaced with a custom `t3admin_role` column (via `manage_users_columns`) that shows the real role and, when a temp grant is active, a second italic line with the temporary role set and remaining time. Hovering shows the exact expiry timestamp.
